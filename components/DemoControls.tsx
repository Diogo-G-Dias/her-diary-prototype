...
export default function DemoControls() {
  const { endConversation, comeBack, resetChat, restartDemo, consolidation, mode, dayOffset, today, thread, endedCount } =
    useDemo();
  const busy = consolidation === 'running';
  const canEnd = thread.some((m) => m.role === 'user') && mode === 'chat' && !busy;
  ...
  return (
    <div className={styles.bar} aria-label="Demo controls">
      ...
      <button type="button" className={styles.btn} onClick={() => void comeBack()} disabled={busy || mode === 'return'}>
        Come back in 2 days
      </button>
